        ...
        paramView.setOnClickListener(new View.OnClickListener() {
            public void onClick(View paramAnonymousView) {
                if (localEpgItem.getIsArchive()) {
                    ...
                    try {
                        String archiveUrl = MyApplication.getInstance().getDbApi().GetArchiveUrl
                                (Integer.valueOf(MyApplication.getInstance().getChId()), Integer.valueOf(localEpgItem.getTimestamp()));
                        if (archiveUrl.contains("error")) {
                            Toast.makeText(MyApplication.getInstance().getUserPage(), "No " +
                                    "entry in the archive", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        Intent localIntent = new Intent();
                        localIntent.setClass(MyApplication.getInstance().getUserPage(), VideoViewPlayer.class);
                        localIntent.putExtra("ch_path", archiveUrl);
                        localIntent.putExtra("ch_name", localEpgItem.getTitle());
                        localIntent.putExtra("ch_id", MyApplication.getInstance().getChId());
                        localIntent.putExtra("type", 2);
                        localIntent.putExtra("service_id", MyApplication.getInstance().getUserPage().iptvServiceId);
                        localIntent.putExtra("timestamp", localEpgItem.getTimestamp());
                        MyApplication.getInstance().getUserPage().startActivity(localIntent);
                    } catch (IOException e) {
                        e.printStackTrace();
